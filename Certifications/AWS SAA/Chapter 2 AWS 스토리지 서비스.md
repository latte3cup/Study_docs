...
-  EFS는 공유 시스템으로 서 다수의 인스턴스를 위한 공통 파일 시스템 역할을 수행
- 주요 속성
  1. 완전관리형
  2. 파일 시스템 액세스 시맨틱 : 일반적인 파일 시스템과 동일
  3. 파일 시스템 인터페이스 
  4. 공유 파일 시스템
  5. 민첩성 및 확장성
  6. 고성능
  7. 고가용성 및 고신뢰성 : 리전 내 다수 AZ에 자동 복제됨  --> 접근 최적화 용도임
- 사용
  파일 시스템 생성 -> 각  AZ에 접근하려는 마운트 타겟 생성 -> EC2에서 마운트

--- 
#### 온프레미스 스토리지와 AWS 통합
- 10TB 이상 전송시 고려
###### AWS Storage Gateway (SGW)
- 내부에 있는 기존 서버나 NAS를 AWS 스토리지(S3, EBS, Glacier 등)와 연결해주는 중간 다리
- 3가지 스토리지 인터페이스 지원
 1. **File Gateway**
  - SMB/NFS 프로토콜 제공
  - **S3에 파일 저장**
 ...
 2. **Volume Gateway**
   - iSCSI 블록 디바이스 제공
  - **EBS Snapshot 형태로 AWS에 백업**
  - Cached / Stored 모드 존재 |
  - 디스크 기반 DB 백업
  - 로컬 블록 저장 + 클라우드 복제
 3. **Tape Gateway**
  - 가상 테이프 장치 (VTL)를 제공
  - **Glacier/Deep Archive에 테이프처럼 저장**
  - 백업 소프트웨어 연동 (NetBackup 등) |
  - 물리 테이프 → 클라우드 마이그레이션
  - 금융/의료/법률 아카이빙

###### AWS Snowball
- 스토리지 디바이스를 이용한 페타바이트 규모의 **물리 장비 기반의 데이터 전송 서비스**

|장비|설명|용량|
|---|---|---|
|**Snowball Edge Storage Optimized**|저장 최적화, 대부분의 데이터 마이그레이션에 사용|최대 **80TB 유효 공간** (장비당)|
|**Snowball Edge Compute Optimized**|EC2/Greengrass 탑재 가능 → 데이터 수집 + 분석|스토리지 + 연산|
|**Snowcone**|소형 장비, 저전력, USB-C|약 8TB 내외|
|❗ **Snowmobile**|**트럭 단위의 이송 시스템**|**최대 100PB** (!)|
