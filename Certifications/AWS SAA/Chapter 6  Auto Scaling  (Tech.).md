...
---
#### 로드 밸런서의 핵심 개념과 용어
- 고가용성, 고확장성의 완전 관리형 서비스
- 핵심 구성 요소
	1. 리스너 : 클라이언트 연결을 받아들이는 진입점. 프로토콜과 포트, 규칙으로 구성되어 있음. 이에 따라 트래픽을 타겟 그룹으로 전달
	2. 타깃 그룹과 타깃 : 로드 밸런서가 실제로 트래픽을 보내는 대상 그리고 그들의 집합. 타겟그룹은 내부적으로 **헬스 체크 (Health Check)** 를 수행해서 정상 타겟에만 요청을 분배
	3. 규칙 : 리스너(Listener)가 받은 요청을 어떤 Target Group으로 보낼지 결정하는 조건과 행동(Condition & Action)을 정의한 것
	   조건 : 경로, 호스트명, 헤더, 쿼리,메서드 등 // 동작 : 리다이렉트, 전달 , 고정 응답
	4. 헬스체크 (Health Check) :  고가용성을 위해 필요한 기능. 타깃과 타깃 그룹이 항상 문제 없이 가동될 수 있도록 미리 정의된, 주기별로 상태를 확인하는 동작

|상태 값(Status)|의미|
|---|---|
|**healthy**|Target이 헬스 체크에 연속 성공하여 정상 상태로 간주됨. 로드 밸런서가 트래픽을 분산함.|
|**unhealthy**|Target이 헬스 체크에 연속 실패하여 비정상 상태로 간주됨. 로드 밸런서가 트래픽 분산에서 제외함.|
|**initial**|Target이 등록된 직후, 아직 헬스 체크가 완료되지 않아 상태를 알 수 없음.|
|**draining**|Target이 Target Group에서 제거 중. 새 연결은 받지 않고, 기존 연결만 처리 후 종료. (Connection Draining / Deregistration Delay 단계)|
|**unused**|Target Group에는 있지만 현재 어떤 로드 밸런서 리스너 규칙에도 연결되지 않아 트래픽을 받지 않는 상태.|
|**unavailable**|헬스 체크를 실행할 수 없는 상태 (예: 잘못된 설정, 포트 차단, 접근 불가).|

“**가외 처리용량**(Overprovisioned Capacity, 여분의 처리 용량)”은 **서비스가 평소 예상 부하보다 더 많은 트래픽을 갑자기 받아도 감당할 수 있도록 확보해 둔 여유 자원**을 의미합니다.

---
#### 다중 AZ 로드밸런스 사용시 나타나는 흔한 문제들

###### 1. 세션 일관성(Session Stickiness) 문제
- 사용자가 AZ-A의 Target에 붙었는데, 다음 요청이 AZ-B로 가면 세션 정보가 날아가는 문제 발생.
- 웹 애플리케이션이 세션을 메모리에 저장할 때 자주 나타납니다.
- **해결책**:
    - ALB의 **스티키 세션(Sticky Session)** 기능 사용 (쿠키 기반).
    - 세션을 **외부 저장소(ElastiCache/Redis, DynamoDB, RDS)**에 저장해서 어느 AZ에서 처리해도 동일 결과 나오게 설계.
###### 2. 크로스 AZ 트래픽 비용 및 지연
- ALB/NLB는 기본적으로 Cross-Zone Load Balancing 기능이 있어, 요청을 다른 AZ의 Target으로도 분배할 수 있습니다.
- 하지만 이 경우 **AZ 간 데이터 전송 비용(유상)**과 **추가 지연**이 발생.
- 특히 대규모 트래픽 서비스에서는 비용 폭탄 가능.
- **해결책**:
    - 가능하다면 각 AZ마다 충분한 Target을 두고, Cross-Zone LB는 비활성화.
    - 어쩔 수 없이 Cross-Zone이 필요하면, 아키텍처 차원에서 비용/지연 감안.
###### 3. 불균형한 트래픽 분산
- 특정 AZ에 더 많은 인스턴스가 있거나, 라우팅 정책이 편중되면 한쪽 AZ Target만 과부하.
- DNS 기반(특히 Route 53 지연시간 라우팅)과 LB 내부 라우팅이 꼬여서 비대칭 발생 가능.
- **해결책**:
    - Target Group에 AZ별로 균등하게 리소스를 배치.
    - CloudWatch 모니터링으로 AZ별 트래픽 편차 감시.
###### 4. DNS 캐싱 문제
- ELB DNS는 다중 AZ의 여러 IP를 반환합니다.
- 클라이언트 DNS가 특정 IP를 오래 캐싱하면, 한 AZ 장애 시 여전히 죽은 IP로 접속 시도.
- 특히 모바일 앱, IoT 디바이스에서 흔히 발생.
- **해결책**:
    - 클라이언트에서 DNS TTL을 짧게 캐싱.
    - 가능하다면 **Route 53 + Health Check**로 장애 시 빠른 우회 제공.
###### 5. Cross-Zone LB
- AWS가 제공하는 **하나의 ELB(Elastic Load Balancer) 논리적 엔드포인트** 앞단에서 작동하는 기능입니다.
- 모든 AZ Target을 풀(pool)처럼 활용하는 기능
- 장점: 균등한 트래픽 분산, 장애 복원력, Auto Scaling 활용
- 단점: Cross-AZ 비용 증가, 지연 시간